...
def render_chart(fig, chart_type: str) -> None:
    st.caption(f"Tipo de grafico: {chart_type}")
    st.plotly_chart(fig, width="stretch")


def chart_selector(label: str, key: str, options: list[str], index: int = 0) -> str:
...
def score_or_scatter_chart(
    data: pd.DataFrame,
    chart_type: str,
    title: str,
):
    if chart_type == "Barras por score":
        fig = px.bar(
            data.sort_values("score_combinado"),
            x="score_combinado",
            y="skill",
            orientation="h",
            color="salario_mediano",
            title=title,
            labels={
                "skill": "Skill",
...
with tab_market:
    ...
    with left:
        ...
        family_chart_type = chart_selector(
            "Ver familias de rol como",
            "chart_family_counts",
            ["Barras horizontales", "Barras verticales", "Donut", "Treemap"],
        )
        fig = ranking_chart(
            family_counts,
            category="job_family",
            ...
            chart_type=family_chart_type,
            color_scale="Teal",
        )
        render_chart(fig, family_chart_type)
        st.info(
    ...
    with right:
        ...
        city_chart_type = chart_selector(
            "Ver ciudades como",
            "chart_city_counts",
            ["Barras horizontales", "Barras verticales", "Donut", "Treemap"],
        )
        fig = ranking_chart(
            ...
            chart_type=city_chart_type,
            color_scale="Viridis",
        )
        render_chart(fig, city_chart_type)
        st.info(
    ...
    sector_chart_type = chart_selector(
        "Ver relacion sector-salario como",
        "chart_sector_salary",
        ["Dispersion", "Barras por salario", "Barras por volumen"],
    )
    if sector_chart_type == "Barras por salario":
        fig = px.bar(
            sector_salary.sort_values("salario_mediano"),
            x="salario_mediano",
            y="industry",
            orientation="h",
            color="ofertas",
            title="Sectores ordenados por salario mediano",
            labels={
    ...
    elif sector_chart_type == "Barras por volumen":
        fig = px.bar(
            sector_salary.sort_values("ofertas"),
            x="ofertas",
            y="industry",
            orientation="h",
            color="salario_mediano",
            title="Sectores ordenados por volumen de ofertas",
            labels={
                "ofertas": "Ofertas con salario",
                "salario_mediano": "Salario mediano",
                "industry": "Sector",
    ...
    render_chart(fig, sector_chart_type)
...
with tab_skills:
    ...
    with left:
        if filtered_skills.empty:
            ...
            skill_chart_type = chart_selector(
                "Ver skills como",
                "chart_skill_counts",
                ["Barras horizontales", "Barras verticales", "Donut", "Treemap"],
            )
            fig = ranking_chart(
                ...
                chart_type=skill_chart_type,
                color_scale="Cividis",
            )
            render_chart(fig, skill_chart_type)
            st.info(
    ...
    with right:
        ...
        if tech_source.empty:
            ...
            tech_chart_type = chart_selector(
                "Ver tecnologias como",
                "chart_tech_rankings",
                ["Barras horizontales", "Barras verticales", "Donut", "Treemap"],
            )
            ...
            fig = ranking_chart(
                ...
                chart_type=tech_chart_type,
                color_scale="Magma",
            )
            render_chart(fig, tech_chart_type)
            st.info(
    ...
    if not used.empty and not wanted.empty:
        ...
        gap_chart_type = chart_selector(
            "Ver brecha tecnologica como",
            "chart_tech_gap",
            ["Barras horizontales", "Barras verticales"],
        )
        if gap_chart_type == "Barras verticales":
            fig = px.bar(
                gap_view,
                x="technology",
                y="gap_wanted_minus_used",
                color="category",
        ...
        render_chart(fig, gap_chart_type)
    ...
    if filtered_skills.empty or not required_columns.issubset(filtered_skills.columns):
        ...
        if skill_salary_base.empty:
            ...
            if skill_salary.empty:
                ...
                skill_salary_chart_type = chart_selector(
                    "Ver demanda-salario como",
                    "chart_skill_salary",
                    ["Dispersion", "Barras por score"],
                )
                fig = score_or_scatter_chart(
                    skill_salary_view,
                    skill_salary_chart_type,
                    "Skills con mejor equilibrio entre demanda y salario",
                )
                render_chart(fig, skill_salary_chart_type)
                st.info(
...
with tab_bias:
    ...
    with left:
        salary_distribution_type = chart_selector(
            "Ver distribucion salarial como",
            "chart_salary_distribution",
            ["Histograma + boxplot", "Boxplot", "Violin"],
        )
        salary_distribution_data = filtered.dropna(subset=["salary_clean"])
        if salary_distribution_type == "Boxplot":
            fig = px.box(
        ...
        elif salary_distribution_type == "Violin":
            fig = px.violin(
                salary_distribution_data,
                y="salary_clean",
                box=True,
        ...
        render_chart(fig, salary_distribution_type)
        st.info(
    ...
    with right:
        ...
        group_salary_chart_type = chart_selector(
            "Ver comparativa salarial como",
            "chart_group_salary",
            ["Boxplot", "Violin", "Barras por mediana"],
        )
        ...
        if group_salary_chart_type == "Violin":
            fig = px.violin(
                box_data,
                x=box_group,
                y="salary_clean",
        ...
        elif group_salary_chart_type == "Barras por mediana":
            salary_by_group = (
                box_data.groupby(box_group, as_index=False)
                .agg(salario_mediano=("salary_clean", "median"), ofertas=("job_id", "count"))
                .sort_values("salario_mediano", ascending=False)
            )
            fig = px.bar(
                salary_by_group,
                ...
                y="salario_mediano",
                color="ofertas",
                title=f"Salario mediano por {box_group}",
                labels={box_group: box_group, "salario_mediano": "Salario mediano", "ofertas": "Ofertas"},
        ...
        render_chart(fig, group_salary_chart_type)
        st.info(
    ...
    missing_chart_type = chart_selector(
        "Ver ausencia de salario como",
        "chart_missing_salary",
        ["Barras horizontales", "Barras verticales", "Treemap"],
    )
    fig = ranking_chart(
        missing,
        category=missing_group,
        value="pct_sin_salario",
        title="Posible sesgo MNAR: grupos con mas salarios ausentes",
        labels={"pct_sin_salario": "% sin salario", missing_group: "Grupo"},
        chart_type=missing_chart_type,
        color_scale="Reds",
    )
    render_chart(fig, missing_chart_type)
...
with tab_quality:
    ...
    with left:
        ...
        nulls_chart_type = chart_selector(
            "Ver nulos como",
            "chart_nulls",
            ["Barras horizontales", "Barras verticales", "Treemap"],
        )
        fig = ranking_chart(
            ...
            chart_type=nulls_chart_type,
            color_scale="Oranges",
        )
        render_chart(fig, nulls_chart_type)
